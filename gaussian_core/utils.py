...
def hf_weightmap(image,threshold=0.75):
    with torch.no_grad():
        f = torch.fft.fft2(image)
        fshift = torch.fft.fftshift(f)

        h, w = fshift.shape[2], fshift.shape[3]
        L = 0.1
        b = (np.floor(np.amin((h, w)) * L))
        c_h = np.floor(h / 2.0)
        c_w = np.floor(w / 2.0)
        h1 = c_h - b
        h2 = c_h + b + 1
        ...
        fshift_h = torch.zeros_like(fshift)
        fshift_h[:, :, int(h1):int(h2), int(w1):int(w2)] = fshift[:, :, int(h1):int(h2), int(w1):int(w2)]
        img = torch.abs(torch.fft.ifft2(torch.fft.ifftshift(fshift_h)))
        img = torch.clip(img, 0, 1)
        binary_img = torch.where(img > threshold, torch.tensor(255, dtype=torch.uint8, device=img.device),
                                 torch.tensor(0, dtype=torch.uint8, device=img.device))
        weight_map = binary_img.float().mean(dim=1, keepdim=True)
...
class Charbonnier(torch.nn.Module):
    ...
    def forward(self, img0, img1):
        return (((img0 - img1) ** 2 + 1e-6) ** 0.5).mean()
...
class Ternary(torch.nn.Module):
    def __init__(self):
        ...
        self.w = np.tile(self.w, (1, in_channels, 1, 1))
        self.w = torch.tensor(self.w).float().cuda()

    def transform(self, img):
        patches = F.conv2d(img, self.w, padding=3, bias=None)
        transf = patches - img.mean(dim=1, keepdim=True)
        # transf = patches - img
...
def training(opt, dataloader, gaussians, model):
    ...
    if (opt.version == 'v1'):
        ...
        if opt.coarse_iters > 0:
            recon(opt, dataloader, gaussians, "coarse", opt.coarse_iters)
        if opt.fine_iters > 0:
            recon(opt, dataloader, gaussians, "fine", opt.fine_iters, model)

def recon(opt, dataloader, gaussians, stage, num_iter, model=None):
    first_iter = 0
    ...
    while True:
        for data in dataloader:
            ...
            prev_viewpoint_cams = [data['prev_camera']]
            ...
            with torch.no_grad():
                for viewpoint_cam in prev_viewpoint_cams:
                    gs_cam = viewpoint_cam

                    render_pkg = render(gs_cam, gaussians, data['time'], background, stage=stage)
                    prev_image = render_pkg["render"]
                    prev_images.append(prev_image.unsqueeze(0))
                    prev_gt_image = viewpoint_cam.original_image.cuda()
                    prev_gt_images.append(prev_gt_image.unsqueeze(0))
            ...
            for viewpoint_cam in viewpoint_cams:
                ...
                if iteration > regular_from:
                    ...
                    with torch.no_grad():
                        ...
                        gaussian_centers_map_z = gaussians.get_points_depth_in_depth_map(fov_camera, depth,
                                                                                         gaussian_centers_in_camera_space,
                                                                                         gs_cam.image_height,
                                                                                         gs_cam.image_width)

                        gaussian_standard_deviations = (
                                gaussians.get_scaling * quaternion_apply(quaternion_invert(gaussians.get_rotation),
                                                                         gaussian_to_camera)
                        ).norm(dim=-1)

                        gaussians_close_to_surface = (
                                                                 gaussian_centers_map_z - gaussian_centers_z).abs() < close_gaussian_threshold * gaussian_standard_deviations
                    ...
                    if n_gaussians_in_sampling > 0:
                        ...
                        sdf_samples_map_z = gaussians.get_points_depth_in_depth_map(fov_camera, depth,
                                                                                    sdf_samples_in_camera_space[
                                                                                        proj_mask], gs_cam.image_height,
                                                                                    gs_cam.image_width)
                        ...
                        closest_min_scaling = gaussians.get_scaling.min(dim=-1)[0][closest_gaussians_idx].detach().view(
                            len(sdf_samples), -1)
                        ...
                        normal_weights = ((sdf_samples[:, None] - gaussians.get_xyz[
                            closest_gaussians_idx]) * closest_gaussian_normals).sum(dim=-1).abs()
            ...
            prev_gt_image_tensor = torch.cat(prev_gt_images, 0)
            ...
            if opt.data == 'scared':
                ...
                depth_loss = F.huber_loss(pred_depth_tensor, gt_depth, delta=0.2)
            ...
            if (stage == 'fine'):
                weight_map = hf_weightmap(gt_image_tensor, 0.7)
                Ll1_h = torch.abs((image_tensor * mask - gt_image_tensor * mask)) * weight
                Ll1_h = Ll1_h * weight_map
                Ll1_h = Ll1_h.mean()
                Ll1 = Ll1 + Ll1_h.detach()
  
                padding_factor = 32
                nearest_size = [int(np.ceil(prev_image_tensor.size(-2) / padding_factor)) * padding_factor,
                                int(np.ceil(prev_image_tensor.size(-1) / padding_factor)) * padding_factor]
                image1 = F.interpolate(prev_image_tensor, size=nearest_size, mode='bilinear',align_corners=True)
                image2 = F.interpolate(image_tensor, size=nearest_size, mode='bilinear',align_corners=True)
                gt_image1 = F.interpolate(prev_gt_image_tensor, size=nearest_size, mode='bilinear',align_corners=True)
                gt_image2 = F.interpolate(gt_image_tensor, size=nearest_size, mode='bilinear',align_corners=True)
                maskr = F.interpolate(mask, size=nearest_size, mode='bilinear',align_corners=True)


                image1 = torch.clip(image1, 0, 1)
                image2 = torch.clip(image2, 0, 1)
                gt_image1 = torch.clip(gt_image1, 0, 1)
                gt_image2 = torch.clip(gt_image2, 0, 1)
                image1 = (image1*255).type(torch.uint8)
                image2 = (image2*255).type(torch.uint8)
                gt_image1 = (gt_image1*255).type(torch.uint8)
                gt_image2 = (gt_image2*255).type(torch.uint8)
                
                with torch.no_grad():
                    image_flow = model(image1, image2,
                                       attn_type='swin',
                    ...
                    gt_image_flow = model(gt_image1, gt_image2,
                                          attn_type='swin',
                                          attn_splits_list=[2, 8],
                                          corr_radius_list=[-1, 4],
                                          prop_radius_list=[-1, 1],
                ...
                #flow_loss =  0.1*(torch.abs(image_flow * maskr - gt_image_flow * maskr)).mean()
                flow_loss = Charbonnier()(image_flow * maskr, gt_image_flow * maskr) + Ternary()(image_flow * maskr, gt_image_flow * maskr).mean()

                Ll1 = Ll1 + 15*flow_loss.detach()
                #Ll1 = Ll1 + flow_loss.detach()
