...
class EndoDataset:
    def __init__(self, opt, device, type='train'):
        ...
        for idx, f in tqdm.tqdm(enumerate(self.images), desc=f'Loading {type} data'):
            ...
            self.cameras.append(Camera(colmap_id=idx, R=R, T=T, FoVx=FovX, FoVy=FovY,
...
class EndoDatasetv2:
    def __init__(self, opt, device, type='train'):
        ...
        if not os.path.exists(os.path.join(self.root_path, 'gt_masks')):
            ...
            gt_mask = True
        poses, _, imgs, masks, depth = _load_data(self.root_path, factor=None, fg_mask=True, use_depth=True,
                                                  gt_mask=gt_mask)  # factor=8 downsamples original imgs by 8x
        ...
        prev_camera = None
        for idx, f in tqdm.tqdm(enumerate(self.images), desc=f'Loading {type} data'):
            current_camera = Camera(colmap_id=idx, R=R, T=T, FoVx=FovX, FoVy=FovY, image=f.to(self.device),
                                    gt_alpha_mask=None, image_name=str(idx), uid=id, data_device=self.device)

            if prev_camera is None:
                prev_camera = current_camera
            self.cameras.append(current_camera)
            self.prev_cameras.append(prev_camera)
            prev_camera = current_camera
    ...
    def collate(self, index):
        ...
        results['camera'] = self.cameras[index[0]]
        results['prev_camera'] = self.prev_cameras[index[0]]
        results['time'] = self.time[index[0]]
...
def _load_data_scared(basedir, factor=None, width=None, height=None, load_imgs=True, fg_mask=False, use_depth=False,gt_mask=True):
    print(11111111111)
    check_colorimg_fn = lambda f, i: f.endswith('JPG') or f.endswith('jpg') or f.endswith('png')
    ...
    rgb_files, new_factor = _preprocess_imgs(basedir, dir_name='images', factor=factor, width=width, height=height,
                                             check_fn=check_colorimg_fn)
    ...
    if fg_mask:
        if gt_mask:
            mask_files, _ = _preprocess_imgs(basedir, dir_name='gt_masks', factor=factor, width=width, height=height,
                                             check_fn=check_maskimg_fn)
        else:
            mask_files, _ = _preprocess_imgs(basedir, dir_name='masks', factor=factor, width=width, height=height,
                                             check_fn=check_maskimg_fn)
    ...
    depth_imgs = None
    if use_depth:
        depth_files, _ = _preprocess_imgs(basedir, dir_name='depth', factor=factor, width=width, height=height,
                                          check_fn=check_depthimg_fn)
    ...
    print('Loaded image data', rgb_imgs.shape)
    return rgb_imgs, mask_imgs, depth_imgs

class ScaredDatasetv2:
    def __init__(self, opt, device, type='train'):
        super().__init__()
        ...
        imgs, masks, depth = _load_data_scared(self.root_path, factor=None, fg_mask=True, use_depth=True,
                                               gt_mask=gt_mask)  # factor=8 downsamples original imgs by 8x
